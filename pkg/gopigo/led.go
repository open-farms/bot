...
func (l *LED) ApplyColor(c color.RGBA, light gopigo3.Led) error {
	l.color = c
	log.Debug().Str("func", "applycolor").Msgf("setting led %v to color %v", light, c)
...
func (l *LED) Blink(frequency time.Duration) error {
	...
	gobot.Every(frequency, func() {
		log.Debug().Str("func", "blink").Msgf("blinking left led to %v", on)
		err := l.driver.SetLED(gopigo3.LED_EYE_RIGHT, 0x00, 0x00, on)
		if err != nil {
			panic(err)
		}

		log.Debug().Str("func", "blink").Msgf("blinking right led to %v", on)
		err = l.driver.SetLED(gopigo3.LED_EYE_RIGHT, 0x00, 0x00, on)
		if err != nil {
			panic(err)
		}
...
func (l *LED) Wink(frequency time.Duration, light gopigo3.Led) error {
	...
	log.Debug().Str("func", "wink").Msgf("setting %s to wink at interval %v", light, frequency)
	gobot.Every(frequency, func() {
